...
def plot_text(
    image, text, origin, color, font=cv2.FONT_HERSHEY_SIMPLEX, fntScale=0.8, thickness=2
):
...
class VideoFrameHandler:
    ...
    def process(self, frame: np.array, thresholds: dict):
        ...
        DROWSY_TIME_txt_pos = (10, int(frame_h // 2 * 1.9))
        ALM_txt_pos = (200, int(frame_h // 2 * 1.0))
        ...
        if results.multi_face_landmarks:
            ...
            frame = plot_eye_landmarks(
                frame, coordinates[0], coordinates[1], self.state_tracker["COLOR"]
            )
            ...
            DROWSY_TIME_txt = (
                f"DROWSY: {round(self.state_tracker['DROWSY_TIME'], 3)} Secs"
            )
            plot_text(frame, EAR_txt, self.EAR_txt_pos, self.state_tracker["COLOR"])
            plot_text(
                frame, DROWSY_TIME_txt, DROWSY_TIME_txt_pos, self.state_tracker["COLOR"]
            )
